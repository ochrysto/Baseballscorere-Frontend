...
export class GamePageService {
  ...
  getDiamondByBattingOrder(battingOrder: number) {
    // const foundDiamond = this.diamonds.find(diamond => diamond.order === battingOrder)
    // return foundDiamond ? foundDiamond : null;
    const index = battingOrder - 1; // Adjust for zero-based indexing
    if (index < 0 || index >= this.diamonds.length) {
      return null; // Or provide a default diamond if needed
    }
    return this.diamonds[index];
  }


  getHomeDiamondByBattingOrder(battingOrder: number) {
    // const foundDiamond = this.diamonds.find(diamond => diamond.order === battingOrder)
    // return foundDiamond ? foundDiamond : null;
    const index = battingOrder - 1; // Adjust for zero-based indexing
    if (index < 0 || index >= this.HomeDiamonds.length) {
      return null; // Or provide a default diamond if needed
    }
    return this.HomeDiamonds[index];
  }
  ...
  offensiveActionsHomeTeam: OffensiveActions[] = [
    {
      name: 'Moser',
      passNumber: 1,
      offensiveActions: [
  ...
  offensiveActionsGuestTeam: OffensiveActions[] = [
    {
      name: 'Riermeister',
      passNumber: 1,
      offensiveActions: [
